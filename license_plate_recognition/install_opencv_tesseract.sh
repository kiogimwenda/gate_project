...
print_message "Downloading OpenCV 5.0.0-alpha and OpenCV contrib..."
cd ~
if [ ! -d "opencv-5.0.0-alpha" ]; then
   wget -O opencv-5.0.0-alpha.zip https://github.com/opencv/opencv/archive/5.0.0-alpha.zip 
fi

if [ ! -d "opencv_contrib-5.0.0-alpha" ]; then
    wget -O opencv_contrib-5.0.0-alpha.zip https://github.com/opencv/opencv_contrib/archive/5.0.0-alpha.zip
fi

print_message "Extracting downloaded zip files..."
unzip -o opencv-5.0.0-alpha.zip
unzip -o opencv_contrib-5.0.0-alpha.zip

print_message "Creating build directory..."
cd ~/opencv-5.0.0-alpha
...
cmake \
    ...
    -D OPENCV_EXTRA_MODULES_PATH=~/opencv_contrib-5.0.0-alpha/modules \
    -D WITH_TBB=ON \
    -D WITH_V4L=ON \
    -D WITH_QT=OFF \
    -D WITH_OPENGL=ON \
...
print_message "Compiling OpenCV 5.0.0-alpha..."
make -j$(nproc)
...
print_message "Installing OpenCV 5.0.0-alpha..."
sudo make install
sudo ldconfig
...
echo -e "${GREEN}OpenCV 5.0.0-alpha and Tesseract have been successfully installed!${NC}"
